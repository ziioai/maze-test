...
可以通过迷宫尺寸、机制复杂度、情境类型、回环率、对象数量、最短路径阈值和文案风格控制难度。这些参数考察的能力并不相同，不能简单合并成一个“地图大小”标签。题面输出不包含答案；参照模拟器则能通过 TypeScript API 提供结构化答案和完整逐步轨迹，便于定位模型第一次发生推理错误的位置。
...
`maze-test` 的难度是多维的。地图更大，不一定比“机制更多、并且要求访问全部对象”的小地图更难。

| 难度维度 | 主要控制参数 | 主要考察内容 |
|---|---|---|
| 路线长度 | `--rows`、`--cols`、种子、`--min-distance`、情境 | 长程位置与状态跟踪 |
| 规则复杂度 | `--complexity` 和机制覆盖参数 | 钥匙、锁、分类宝物、生命、中毒、速度和耗时 |
| 路线覆盖范围 | `--scenario` | 行动序列实际经过哪些对象和边界情况 |
| 拓扑结构 | `--braid` | 替代路线和门能否绕行；难度不一定单调增加 |
| 输出负担 | 问题字段数和题面长度 | 模型需要维护并最终报告多少状态 |
| 语言变化 | `--lang`、`--style` | 跨语言和措辞鲁棒性，而不是迷宫机制本身 |

其中，`advanced` 负责增加规则种类，`mechanism-tour` 则强制路线访问全部生成对象。两者结合后的难度远高于单独使用其中一个。待测回答固定后，可以用答案 JSON 中的 `actionCount` 衡量这道题实际包含多少条原子移动；无泄漏评测中，不要提前查看这个值。

建议从以下档位开始：

| 档位 | 参数 | 适用场景 |
|---|---|---|
| 快速检查 | `21×21`、`basic`、`success` | 快速检查空间推理和基础记账 |
| 高级规则 | `21×21`、`advanced`、`mechanism-tour` | 在可控路线长度内覆盖全部规则 |
| 长程挑战 | `51×51`、`advanced`、`mechanism-tour` | 数千步移动和完整状态账本 |
| 极限压力 | `101×101`、`advanced`、`mechanism-tour` | 刻意测试模型的完成边界，可能出现弃答 |

例如：

```sh
npx maze-test question --seed 2101 --rows 21 --cols 21 \
  --complexity advanced --scenario mechanism-tour --lang zh
```

当前的双种子实测能够说明，尺寸本身不足以代表难度：

| 试题组合 | 实测原子移动数 | Astra/high | Sol/high |
|---|---:|---:|---:|
| 51×51、basic、success | 598–620 | 100% | 100% |
| 51×51、advanced、mechanism-tour | 2,548–3,612 | 95.2% | 弃答 |
| 101×101、basic、success | 2,272–2,458 | 100% | 100% |
| 101×101、advanced、mechanism-tour | 10,682–10,824 | 弃答 | 弃答 |

这些数字只是两个种子的校准示例，不是普遍的模型排名。公平比较时，应使用相同种子和完整参数，分别报告“是否完成”和“字段准确率”，并且每次只调整一个难度维度。完整记录见[模型评测报告](./benchmarks/default-maze-model-comparison-2026-09-17.md)。
...
该 Skill 会指导 Agent 选择合适的难度档位，在不泄漏答案的前提下生成题目、固定待测回答、生成参考答案、逐项精确评阅，并保存可复现的评测记录。它会把完成状态与字段准确率分开记录，避免隐藏弃答或输出截断。
