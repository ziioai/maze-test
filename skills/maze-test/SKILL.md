...
Give only the generated question to the model or person being evaluated. Ask for all eleven requested fields and save the response verbatim before continuing.

When solving the question yourself, explicitly state the eleven answers before retrieving the reference answer. Explanations may be useful, but they do not replace the requested fields.
...
Score the eleven requested answer fields independently. Do not score `blockedAfterDeath`; it is diagnostic metadata and is not one of the standard questions.
...
- Per-field correctness and total score out of 11.
